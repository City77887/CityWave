...
const { collection, onSnapshot, doc, setDoc, deleteDoc } = firestoreModule as any;
    ...
    <nav className="bg-gray-900/80 border-b border-gray-800 sticky top-0 z-50 backdrop-blur-xl">
      <div className="max-w-7xl mx-auto px-4 h-20 flex justify-between items-center text-left">
        <Link to="/" className="flex items-center gap-2 group">
          ...
          <span className="font-black text-2xl tracking-tighter text-white uppercase">CITYWAVE <span className="text-indigo-500">EVENTS</span></span>
        </Link>
          ...
          {currentAdmin ? (
            <div className="flex items-center gap-3 bg-gray-800 border border-gray-700 pl-4 pr-2 py-1.5 rounded-2xl">
               <div className="flex flex-col items-end">
                 <span className="text-[10px] font-black text-indigo-500 uppercase leading-none mb-0.5">{currentAdmin.isMain ? 'Glavni Admin' : 'Admin Panel'}</span>
                 <span className="text-sm font-bold text-white leading-none">{currentAdmin.displayName || currentAdmin.username}</span>
               </div>
            ...
            <Button onClick={onLoginClick} variant="secondary" className="rounded-2xl">
              <ShieldCheck className="w-5 h-5" /> Admin Prijava
            </Button>
          )}
        </div>
      </div>
    ...
    <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center z-[250] p-4">
      <div className="bg-gray-900 border border-gray-800 rounded-3xl w-full max-w-md p-10 shadow-2xl space-y-8">
        <div className="flex justify-between items-center">
          <h2 className="text-2xl font-black text-white uppercase text-left">Stol: {table.name}</h2>
        ...
        <div className="lg:col-span-7 space-y-10">
           ...
           {isAdmin && isOwner && (
             <div className="flex gap-3 bg-indigo-600/5 p-6 rounded-3xl border border-indigo-600/10">
                <input className="flex-1 bg-gray-900 border border-gray-800 rounded-2xl px-6 font-bold outline-none focus:ring-2 focus:ring-indigo-500 text-white" placeholder="Naziv novog stola..." value={newT} onChange={e => setNewT(e.target.value)} />
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-2xl flex items-center justify-center z-[300] p-4">
      <div className="bg-gray-900 p-12 rounded-[3rem] border border-gray-800 w-full max-w-md shadow-2xl">
        <div className="flex flex-col items-center mb-10 text-center"><ShieldCheck className="w-14 h-14 text-indigo-500 mb-4" /><h2 className="text-3xl font-black text-white uppercase tracking-tighter">Admin Panel</h2></div>
...
const AdminManagementModal = ({ isOpen, onClose, currentAdmins, onAddAdmin, onDeleteAdmin }: any) => {
  ...
  const [n, setN] = useState(''); // New display name state
  const [p, setP] = useState('');
  if (!isOpen) return null;
  return (
    <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center z-[200] p-4">
      <div className="bg-gray-900 border border-gray-800 rounded-3xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl text-left">
        ...
        <div className="p-10 overflow-y-auto space-y-12">
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 items-end">
              <Input label="Ime i Prezime" placeholder="Npr. Marko Marić" value={n} onChange={e => setN(e.target.value)} />
              <Input label="Korisničko Ime" placeholder="Korisnik" value={u} onChange={e => setU(e.target.value)} />
              <Input label="Lozinka" placeholder="Lozinka" type="text" value={p} onChange={e => setP(e.target.value)} />
              <Button variant="success" className="mb-4 h-12 rounded-xl" onClick={() => { if(u && p && n){ onAddAdmin(u, n, p); setU(''); setP(''); setN(''); } }}><UserPlus className="w-4 h-4" /> Kreiraj</Button>
            </div>
          </section>
          <section>
              ...
              {currentAdmins.filter((a: any) => !a.isMain).map((admin: any) => (
                <div key={admin.id} className="flex items-center justify-between p-5 bg-gray-800/40 rounded-3xl border border-gray-800">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-2xl bg-indigo-500/10 flex items-center justify-center text-indigo-400 font-black">{(admin.displayName || admin.username)[0].toUpperCase()}</div>
                    <div><p className="font-black text-white">{admin.displayName || admin.username}</p><p className="text-[10px] text-gray-500 font-mono tracking-widest uppercase">User: {admin.username} | Pass: {admin.password}</p></div>
                  </div>
...
const AdminCreate = ({ currentAdmin, onSave }: any) => {
  ...
  return (
    <div className="max-w-2xl mx-auto bg-gray-900 p-12 rounded-[3.5rem] border border-gray-800 shadow-2xl space-y-8 text-left">
       ...
       <Input label="URL Slike" value={f.imageUrl} onChange={e => setF({...f, imageUrl: e.target.value})} />
       <div className="grid grid-cols-2 gap-4">
         <Input label="Min. Serial" type="number" value={f.minS} onChange={e => setF({...f, minS: parseInt(e.target.value)})} />
         <Input label="Max. Serial" type="number" value={f.maxS} onChange={e => setF({...f, maxS: parseInt(e.target.value)})} />
       </div>
...
const App = () => {
  ...
  useEffect(() => {
    const unsubE = onSnapshot(collection(db, "events"), (s: any) => setEvents(s.docs.map((d: any) => d.data() as EventData)));
    const unsubA = onSnapshot(collection(db, "admins"), (s: any) => setAdmins(s.docs.map((d: any) => d.data() as AdminUser)));
    return () => { unsubE(); unsubA(); };
  }, []);

  const handleLogin = async (u: string, p: string) => {
    if (u === 'admin' && p === '13377331LL') { 
      setCurrentAdmin({id:'root', username:'admin', displayName: 'Glavni Administrator', password:'***', isMain:true}); 
      return true; 
    }
  ...
  return (
    <HashRouter>
      ...
      <AdminManagementModal 
        ...
        onAddAdmin={async (u:string, n:string, p:string) => { 
          const id = `adm-${Date.now()}`; 
          await setDoc(doc(db, "admins", id), {id, username:u, displayName: n, password:p, isMain:false}); 
        }} 
